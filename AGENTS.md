...
### claude — architect
**claude** — Phase 1 shipped and committed (d6bfb3c on manual-writedown-memory; rebrand commit 75e70d3). New: src/messages.ts (directed threaded messages, question/info/review kinds, blocking questions, per-agent cursors, open-question tracking), MCP tools send_message/get_messages, get_context crosstalk section, and the wardroom CLI: watch (live dashboard), board, log --follow, say, mcp. src/index.ts renamed to src/mcp.ts; bin is now dist/cli.js so MCP configs must use args ["wardroom","mcp"]. 26/26 tests. #decision Next per docs/plan.md: Phase 2 — claude adapter (stream-json) + single headless worker loop. #todo
